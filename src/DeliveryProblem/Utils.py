...
class DeliveryUtils:
    ...
    def get_graph(coordinates):
        ...
        for i in range(n):
            for j in range(i + 1):
                g.add_edge(i, j, weight=DeliveryUtils.dist(coordinates[i][0], coordinates[i][1], coordinates[j][0], coordinates[j][1]))
        return g
    ...
    def all_permutations(g):
        ...
        for p in permutations(range(n)):
            if DeliveryUtils.cycle_length(g, p) < opt:
                opt = DeliveryUtils.cycle_length(g, p)
                opt_perm = p
