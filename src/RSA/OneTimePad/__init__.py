# This notebook shows examples of One-time Pad encryption and partial decryption of the messages when the same key is
# used more than once for encryption.
# Before using XOR operation, we are going to convert plaintext messages to hex strings. This way, each character is
# represented by its ASCII code - a number from 0 to 255. In the hexadecimal system, any such number has two digits.
# We write these two hexadecimal digits instead of the initial character, like "6f" instead of letter "o", where 'f'
# corresponds to hexadecimal number 15. We can also convert back from hex strings to the regular strings.
# These conversions are needed for demonstration purposes only: to avoid using unreadable/invisible characters in the
# strings we work with.
...
# To encrypt, we need to Xor the message with the key and get the ciphertext. However, before doing that we are going
# to convert them both to hex strings. To decrypt, we need to xor the ciphertext with the key. However, before doing
# that we need to convert key to hex, and after doing that we need to convert the resulting hex string to the regular
# string, so that we get the initial message and not its hex form.
...
# Now we can see that one of the messages has "oy th" starting from position 5, or " th" starting from position 7, and
# one of the messages has " the " startin from position 7. We could guess that this is the same message which
# has "oy the " starting from position 5.

TryGuessingSubstring("oy the ", len(message1), xor_messages)

# Now we've guessed parts of both initial messages. If we had even more ciphertexts of different messages encypted with
# the same key, by xoring each pair the same way we could reconstruct more.
